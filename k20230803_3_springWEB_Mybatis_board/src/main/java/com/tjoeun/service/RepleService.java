...
public class RepleService implements MvcBoardService {
	...
	public void execute(Model model) {
		...
		logger.info("RepleService 클래스의 execute() 메소드 실행");
		
		Map<String, Object> map = model.asMap();
		HttpServletRequest request = (HttpServletRequest) map.get("request");
		
		// Model 인터페이스 객체에 저장되서 넘어온 HttpServletRequest 인터페이스 객체에서 
		// 원본 글번호, 글그룹, 글레벨, 같은 글 글그룹에서 글 출력순서, 답글 작성자 이름,
		// 답글 제목, 답글 내용을 받는다.
		...
		// HttpServletRequest 인터페이스 객체에서 받은 데이터를 MvcBoardVO 클래스의 bean에
		// 넣어준다.
