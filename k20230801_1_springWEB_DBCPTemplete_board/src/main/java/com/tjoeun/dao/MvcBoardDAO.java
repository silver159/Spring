...
// MvcBoardDAO 클래스의 bean이 생성될 때 오라클과 연결한다. 기본 생성자
	public MvcBoardDAO() {
		try {
			template = Constant.template;
...
*/
// 여기까지는 DBCP 방식을 사용하는 객체를 초기화하는 부분이므로 JdbcTemplate으로 코드 변환이
// 완료되면 모두 주석으로 처리한다.
// =========================================================================================	
...
// insert, delete, update sql 명령을 실행하는 메소드의 인수로 넘어온 데이터가 중간에 값이
// 변경되면 안되기 때문에 JdbcTemplate에서는 insert, delete, update sql 명령을 실행하는
...
//		}
		...
		// queryForInt(): 테이블의 내용이 갱신되지 않는 sql 명령 => select 
		// => 결과가 정수일 경우 (count(*))
		// queryForObject(): 테이블의 내용이 갱신되지 않는 sql 명령 => select 
		// => 결과가 1건일 경우
		
		// update(sql 명령, "?"를 채운다.)
		// update() 메소드의 2번째 인수로 PreparedStatementSetter 인터페이스 객체를 익명으로
...
//		Connection conn = null;
//		PreparedStatement pstmt = null;
//		ResultSet rs = null; // select sql 명령시 결과를 저장할 때 필요
//		// 테이블에 저장된 전체 글의 개수를 저장해서 리턴시킬 변수를 선언한다.
//	    int count = 0; 
//		try {
//			conn = dataSource.getConnection();
...
//			
//			// 테이블에 저장된 글의 개수는 null이 나올리가 없으므로 조건 비교는 
//			// 필요없다.
//			rs.next();
//			count = rs.getInt(1); // 첫번째 열
//			
//		} catch (SQLException e) {
//			e.printStackTrace();
//		}finally {
...
//		ResultSet rs = null;
//		
//		// 1페이지 분량의 글 목록을 저장해 리턴시킬 ArrayList 선언
...
//			rs = pstmt.executeQuery();
//			
//			// ResultSet 객체에 저장된 1페이지 분량의 글 목록을 저장할 ArrayList 
//			// 객체 생성
...
//		return list;
		...
		// select sql 명령 실행 결과를 BeanPropertyRowMapper 클래스 생성자의 인수인 
		// MvcBoardVO 클래스로 넘겨서 sql 명령 실행 결과를 저장시켜 리턴한다.
		// query 메소드 실행 결과는 List 인터페이스 타입으로 리턴되기 때문에 메소드의
		// 리턴 타입인 ArrayList<MvcBoardVO>로 형변환이 필요하다.
		// 데이터베이스 테이블의 필드이름과 BeanPropertyRowMapper 클래스 생성자의 인수로
		// 전달되는 클래스의 필드 이름과 같아야 정상적으로 처리된다.
...
//		ResultSet rs = null;
//		
//		// 조회수를 증가시킨 글 1건을 얻어와 저장시켜 리턴할 객체 선언
...
//			rs = pstmt.executeQuery();
//			
//			// ResultSet 객체에 저장된 글 1건을 리턴시키기 위해서 MvcBoardVO 클래스
//			// 객체에 저장한다.
//			if (rs.next()) {
//				AbstractApplicationContext ctx = new GenericXmlApplicationContext("classpath:applicationCTX.xml");
...
//		});
		...
		// 문자열이 들어갈려면 ''안에 넣어주어야 한다. '"+subject+"' '%s'
//		String sql = "update mvcboard set subject = '"+subject+"', content = '"+
//					content + "' where idx =" + idx;
		String sql = String.format("update mvcboard set subject = '%s', content = '%s' "+
					"where idx = %d", subject, content, idx);
		template.update(sql);
...
//				mvcBoardVO.getContent() + "' where idx =" + mvcBoardVO.getIdx();
		String sql = String.format("update mvcboard set subject = '%s', content = '%s' "+
				"where idx = %d", mvcBoardVO.getSubject(), mvcBoardVO.getContent(), mvcBoardVO.getIdx());
		template.update(sql);
...
//			}
//		});
//		String sql = "delete from mvcboard where name = '" + name + "'";
...
//		String sql =" update mvcboard set seq = seq + 1 where gup = "+ hmap.get("gup")+ 
//					" and seq >= " + hmap.get("seq");
		String sql = String.format(" update mvcboard set seq = seq + 1 where gup = %d and seq >= %d",
					hmap.get("gup"), hmap.get("seq"));
		template.update(sql);
	}
