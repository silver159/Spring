...
//		logger.info("조회수를 증가시킨 글번호: {}", request.getParameter("idx"));
		...
		// Model 인터페이스 객체에 저장되서 넘어온 HttpServletRequest 인터페이스
		// 객체에서 조회수를 증가시킨 글 번호를 받는다.
		int idx = Integer.parseInt(request.getParameter("idx"));
		
		// 조회수를 증가시킨 글 1건을 얻어오는 메소드를 실행해서 얻어온 글을 
		...
		// MvcBoardVO 클래스 객체로 넘어오기 때문에 bean을 만들 필요가 없다.
		MvcBoardVO mvcBoardVO = mvcBoardDAO.selectByIdx(idx);
//		logger.info("{}", mvcBoardVO);
		
		// contentView.jsp로 넘겨줄 글 1건, 작업후 돌아갈 페이지 번호, 줄바꿈에 
