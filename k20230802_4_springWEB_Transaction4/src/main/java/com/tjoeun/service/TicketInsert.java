...
public class TicketInsert implements TransactionService {
	...
	public void execute(final CardVO cardVO) {
		...
		// ticket.jsp의 입력과 상관없이 외부 트랜잭션이 있을 때와 없을 때를 구분해서 
		// 트랜잭션을 실행시켜 보자.
		
		// REQUIRED(기본값, 0) <=> REQUIRES_NEW(3)
		
		// REQUIRED
		// 외부 트랜잭션이 존재한다면 외부 트랜잭션으로 합류하고 외부 트랜잭션이 없다면 새로운 
		// 트랜잭션을 생성한다.
		// 중간에 롤백이 발생한다면 모두 하나의 트랜잭션이기 때문에 진행 상황이 모두 롤백된다.
		
		// REQUIRES_NEW
		// 무조건 새로운 트랜잭션을 생성하고 각각의 트랜잭션이 롤백되더라도 서로 영향을 주지 
		// 않는다.
		...
		// SUPPORTS
		// 외부 트랜잭션이 있다면 합류하고 진행 중인 외부 트랜잭션이 없다면 트랜잭션을 
		// 생성하지 않는다.
		
		// NOT_SUPPORTED
		// 외부 트랜잭션이 있다면 보류시키고 진행 중인 외부 트랜잭션이 없다면 트랜잭션을 
		// 생성하지 않는다. => 내부 트랜잭션이 작동하지 않는다.
		
		
		// MANDATORY(2) <=> NEVER(5)
		
		// MANDATORY
		// 외부 트랜잭션에 합류한다. 만약 외부 트랜잭션이 없다면 예외를 발생시킨다.
		
		// NEVER
		// 트랜잭션을 생성하지 않고 외부 트랜잭션이 존재한다면 예외를 발생시킨다.
		
		
		// servlet-context.xml 파일의 propagationBehavior 속성 지정에 따라 트랜잭션 처리가
		// 달라진다.
		// 외부 트랜잭션(transactionTemplate2) 0, 내부 트랜잭션(transactionTemplate) 0이면
		// 모두 적용된다.
		// 외부 트랜잭션이 없고, 내부 트랜잭션(transactionTemplate) 0이면 내부 트랜잭션이 정상
		// 처리된다.
		// 외부 트랜잭션이 있고, 내부 트랜잭션이 1이면 모두 적용된다.
		// 외부 트랜잭션이 없고, 내부 트랜잭션이 1이면 모두 적용되지 않는다. 5가 들어간다ㅠ
		// 외부 트랜잭션이 있고, 내부 트랜잭션이 2이면 모두 적용된다.
		// 외부 트랜잭션이 없고, 내부 트랜잭션이 2이면 예외가 발생된다. IllegalTransactionStateException
