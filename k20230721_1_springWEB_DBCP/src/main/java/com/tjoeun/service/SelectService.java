...
public class SelectService implements MvcBoardService {
	...
	public void execute(Model model) {
		...
		// HttpServletRequest 인터페이스 객체에 저장되어 넘어오는 화면에 표시할 페이지 번호를
		// 받는다.
		int currentPage = 1;
		try {
...
//		logger.info("전체 글의 개수: {}", totalCount);
		...
		// 1페이지 분량의 글 목록과 페이지 작업에 사용할 8개의 변수를 기억하는
		// MvcBoardList 클래스의 bean을 얻어온다.
		MvcBoardList mvcBoardList = ctx.getBean("mvcBoardList", MvcBoardList.class);
		
		// jsp로 작업할 때 처럼 생성자를 사용해서 초기화가 불가능하므로 
		// MvcBoardList 클래스의 bean을 얻어온 후 8개의 변수를 초기화시키는 메소드를
		// 초기화시키는 메소드를 실행한다.
		mvcBoardList.initMvcBoardList(pageSize, totalCount, currentPage);
//		logger.info("{}", mvcBoardList);
		
		// MvcBoardList 클래스의 1페이지 분량의 글 목록을 기억하는 ArrayList에 
		// 1페이지 분량의 글 목록을 얻어와서 넣어주는 메소드를 실행한다.
