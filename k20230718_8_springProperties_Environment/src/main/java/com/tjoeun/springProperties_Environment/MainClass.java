...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// 아래와 같이 DI 컨테이너를 다시 만들면 admin.properties 파일의 properties
		// 정보가 존재하지 않는다.
//		AbstractApplicationContext gCtx = new GenericXmlApplicationContext("classpath:applicationCTX.xml");
		
		// DI 컨테이너를 다시 만들어 환경설정 정보를 얻어오면 addLast() 메소드를
		// 사용해 추가한 properties 정보가 없기 때문에 ConfigurableApplicationContext
		// 인터페이스 타입 객체의 환경 설정 정보를 그대로 사용하기 위해서 xml 
		// 파일에서 bean 설정 정보를 얻어오는 GenericXmlApplicationContext 클래스  
		// 타입으로 형변환 시켜 사용한다.
		// 즉, 기존의 DI 컨테이너를 그대로 사용한다. 
